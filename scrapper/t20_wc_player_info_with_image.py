...
def extract(url):
    ...
    script_tags = soup.find_all("script", string=re.compile(r"peerUrls"))
    try:
        img_url = json.loads(
            str(script_tags).split('[<script id="__NEXT_DATA__" type="application/json">')[1].split("</script>]")[0])[
            "props"]["appPageProps"]["data"]["player"]["image"]["url"]
    except:
        img_url = ""
    ...
    return {
        "name": name,
        "team": team,
        "image": "https://p.imgci.com" + img_url,
...
with open('../datasets/t20_wc_players_info_with_image.json', 'w') as file:
